...
func TestABackgroundProcessCannotOutliveASuccessfulCommand(t *testing.T) {
	directory := t.TempDir()

	result := run(t, directory,
		`(sleep 1; echo escaped > survived) </dev/null >/dev/null 2>&1 &`, sandbox.Policy{})
	if result.Code != 0 {
		t.Fatalf("the shell did not finish successfully: %q", result.Output)
	}

	time.Sleep(1500 * time.Millisecond)
	if _, err := os.Stat(filepath.Join(directory, "survived")); err == nil {
		t.Error("a background process survived the command")
	} else if !os.IsNotExist(err) {
		t.Fatalf("could not inspect the background process's target: %v", err)
	}
}

func TestBackgroundProcessesDieWithTheirProcessSet(t *testing.T) {
	directory := t.TempDir()
	policy := sandbox.Policy{ // no scratch: what outlives the command is written to the directory
		Background: true,
	...
	command := `(setsid sh -c 'sleep 0.2; echo alive > alive') </dev/null >/dev/null 2>&1 &`
	result, err := processes.Run(context.Background(), directory, command, policy)
	if err != nil || result.Code != 0 {
		t.Fatalf("the background command did not start: %v, %q", err, result.Output)
	}

	time.Sleep(400 * time.Millisecond)
	if _, err := os.Stat(filepath.Join(directory, "alive")); err != nil {
		t.Fatalf("the process did not outlive its command: %v", err)
	}

	command = `(setsid sh -c 'sleep 0.4; echo escaped > escaped') </dev/null >/dev/null 2>&1 &`
	result, err = processes.Run(context.Background(), directory, command, policy)
	if err != nil || result.Code != 0 {
		t.Fatalf("the second background command did not start: %v, %q", err, result.Output)
	}
	names, err := processes.Disable()
	if err != nil {
		t.Fatalf("the process set was not stopped: %v", err)
	}
	if len(names) == 0 {
		t.Error("the process set did not name what it stopped")
	}

	time.Sleep(600 * time.Millisecond)
	if _, err := os.Stat(filepath.Join(directory, "escaped")); err == nil {
		t.Error("a process survived its disabled process set")
	} else if !os.IsNotExist(err) {
		t.Fatalf("could not inspect the background process target: %v", err)
	}
}
