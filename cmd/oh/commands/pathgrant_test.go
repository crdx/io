...
func TestGrantCommandPreservesSpacesInThePath(t *testing.T) {
	...
	context, err := invokePathGrantCommand(t, grants, "/grant write /reference/path with spaces")
	if err != nil {
		t.Fatal(err)
	}
	want := []pathgrant.Grant{{Path: "/reference/path with spaces", Access: pathgrant.WriteAccess}}
...
func TestGrantCommandRejectsAnUnknownAccess(t *testing.T) {
	...
	_, err := invokePathGrantCommand(t, grants, "/grant execute /reference")
...
func TestGrantsCommandListsTheCurrentState(t *testing.T) {
	...
	*current = []pathgrant.Grant{
		...
		{Path: "/write", Access: pathgrant.WriteAccess},
	}
