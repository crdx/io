...
func TestTheLastTurnSegmentSaysHowFastItCameBack(t *testing.T) {
	if got := drawn(t, lastTps.New(func() (float64, bool) { return 42.4, true })); got != "~42t/s" {
		t.Errorf("expected a whole rate above ten, got %q", got)
	}

	if got := drawn(t, lastTps.New(func() (float64, bool) { return 4.25, true })); got != "~4.2t/s" {
		t.Errorf("expected a tenth of a token below ten, got %q", got)
	}
}

func TestTheLastTurnSegmentSaysNothingBeforeTheFirstTurnIsOver(t *testing.T) {
	if got := drawn(t, lastTps.New(func() (float64, bool) { return 0, false })); got != "" {
		t.Errorf("expected nothing before there is anything to say, got %q", got)
	}
}
