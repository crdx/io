...
func TestTheGlobalContextReplacesTheBuiltInOpeningButKeepsTheHarnessState(t *testing.T) {
	...
	got, contextFiles, err := loadContext(root, workspace, capRead, configuredPaths{}, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.HasPrefix(got, "You are a deliberately custom assistant.") {
...
func TestAMissingGlobalContextUsesTheBuiltInOpening(t *testing.T) {
	...
	got, _, err := loadContext(root, workspace, capRead, configuredPaths{}, nil)
	if err != nil {
		t.Fatal(err)
	}
	if !strings.HasPrefix(got, defaultGlobalContext) {
...
func TestConfiguredPathsAreDisclosedInTheHarnessContext(t *testing.T) {
	...
	got := harnessContext("/workspace", capRead|capWrite, paths)
...
func TestTheSkillCatalogueIsAppendedToTheContext(t *testing.T) {
	...
	got, _, err := loadContext(root, workspace, capRead, configuredPaths{}, []skill.Skill{{
