...
| Layer     | Path                                    | Scope                    |
|-----------|-----------------------------------------|--------------------------|
| Defaults  | `cmd/oh/config/defaults.toml`, embedded | Every session            |
| Global    | `~/.config/org.crdx/oh/config.toml`     | Every session            |
| Workspace | `<workspace>/oh.toml`                   | Sessions in that project |

Global and workspace files are watched and auto-reload. A reload applies live to the theme, bar, snippets, permissions, streaming, editor command, and tool output cap. Sandbox grants, capability defaults, the toolbox, and the model reach the next session only.
...
Some styling is fixed in the binary — the italics on reasoning and preview hints, the bold on a heading, the simulation gradient — and no theme key reaches it. `cmd/oh/style/theme.go` and `style.go` are canonical.
...
`[sandbox]` decides which paths exist; `[caps]` decides what may be done with them. A path absent from `[sandbox]` is unreachable however generous the caps.

- `read`, `write`, `exec` — grant those operations on paths outside the workspace
- `path` — grant exec, and append the directory to the shell's `PATH`
- `home` — expose a named file from the real home, otherwise replaced by the agent's own
...
This one is built in, written to `<state>/skills/oh/SKILL.md` at startup and rewritten when the binary's content differs; `<state>` is `OH_STATE_DIR` or `~/.local/state/org.crdx/oh`. Change it at `cmd/oh/skill/skills/oh/SKILL.md` in the repository; the next startup overwrites any edit to the copy.
...
oh is built from the `crdx.org/io` module, under `cmd/oh`. Read it to settle a question this skill leaves open.
...
Name the capabilities the change adds, whether it lands now or at the next startup, and that a workspace file persists into every future session in that project.
